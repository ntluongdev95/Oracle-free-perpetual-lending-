...
contract LenderTest is Test{
    ...
    function test_repay () public {
        ...
        loanIds[0] = 0;
        loanToken.mint(BORROWER,10*10**17);
        loanToken.approve(address(lender),2000*10**18);
        collateralToken.approve(address(BORROWER),100*10**18);
        vm.warp(3 days);
        lender.repay(loanIds);

       console.log(loanToken.balanceOf(address(BORROWER)));
    ...
    function test_RevertUnauthorized() public {
         test_borrow();
          vm.expectRevert(Lender.Unauthorized.selector);
    ...
    function test_buyLoan () public {
     ...
     function testFail_buyLoanTooLate() public {
        ...
        loanIds[0] = 0;
         vm.startPrank(LENDER1);
        lender.startAuction(loanIds);
        vm.expectRevert(Lender.AuctionEnded.selector);
        vm.startPrank(LENDER2);
     ...
     function test_ (, , , , , , , , uint256 auctionStartTimestamp, ) = lender.loans(0);

        assertEq(auctionStartTimestamp, type(uint256).max); () public{
         test_borrow();
