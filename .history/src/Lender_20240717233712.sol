...
contract Lender is Ownable {
    ...
    function updateInterestRate(bytes32 poolId, uint16 interestRate) external {
        if (pools[poolId].lender != msg.sender) revert Unauthorized();
    ...
    /*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/
     function isBorrowAmountValid(
        ...
        // Calculate 75% of normalized collateral amount
        uint256 maxBorrowAmount = (normalizedCollateralAmount * maxLoanRatio) / 10000;

        // Check if normalized borrow amount is less than or equal to 75% of normalized collateral amount
